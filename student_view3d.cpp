...
void View3D::paint( float x, float y, float compass_direction )
{
    ...
    for( my = 0; my < map->getHeight(); my++ )
        for( mx = 0; mx < map->getWidth(); mx++ )
        {
            ...
            if(c.isWall()){
                paintParedeXY(mx, my, VIEW3D_IX_TEXTURE_WALL);
            }
            else if(c.isFloor()){
                paintChaoXY(mx, my, VIEW3D_IX_TEXTURE_FLOOR, VIEW3D_IX_TEXTURE_CEILING);
            }
            else if(c.isDoor()){
                paintParedeXY(mx, my, VIEW3D_IX_TEXTURE_WALL);
            }
...
void View3D::paintParedeXY(int x, int y, GLuint textura){
    ...
    int z=0;    //para colocar cada cubo no "chao"... alterar para outras alturas
...
void View3D::paintChaoXY(int x, int y, GLuint texturaBaixo, GLuint texturaCima){
...
/* para pintar uma parede
    recebe      as coordenadas de onde vai partir, o x e o z (y é sempre entre 0 e 1)
                o index da textura que vai pintar
...
/* para pintar chão
    recebe      as coordenadas de onde vai partir, o x e o z (y é sempre entre 0 e 1)
                os index das texturas que vai pintar
